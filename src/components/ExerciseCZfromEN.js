...
const ExerciseENfromCZ = () => {
  ...
  const submitForm = (e) => {
    ...
    const evaluateAndSetState = () => {
      try {
        if (wordEN.toLowerCase() === answer.toLowerCase()) {
          setCorrectly(true);
          ...
          setCorrectly(false);
          setRightAnswer(
            `Skoro správně, přesná odpověd je ${wordEN}. [${accent}]`
          );
  ...
  <div className="md:min-w-[600px] lg:min-w-[800px] xl:min-w-[1000px] h-[500px] min-w-[300px] bg-white shadow-xl border border-gray-200 m-auto p-6 flex items-center justify-center flex-col rounded-lg ">
    {start ? (
      <button
